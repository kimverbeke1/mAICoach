...
LET OP (2026-09-06): dit bestand is NIET hetzelfde als opponent_dossier.py.
opponent_scout.py = vindt de vorige opstelling van een tegenstander +
scrapet ontbrekende tegenstander-spelers (gebruikt in de Opstelling-analyse
pagina, via `import opponent_scout as osc`).
opponent_dossier.py = apart bestand voor het volledige dossier van één
tegenstander (via `import opponent_dossier as od`). Beide bestanden horen
naast elkaar te bestaan, niet in elkaars plaats.
"""
import re
import sys
import time
